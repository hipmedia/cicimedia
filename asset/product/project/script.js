document.addEventListener('DOMContentLoaded', () => {
    ...
    // (ĐÃ THÊM) Khởi tạo thư viện Animate On Scroll (AOS)
    AOS.init({
        duration: 800, // Thời gian hiệu ứng (ms)
        once: true, // Chỉ chạy hiệu ứng một lần
        offset: 100, // Kích hoạt hiệu ứng khi cách phần tử 100px
    });
    ...
    if (filterContainer) {
        ...
        const portfolioItems = document.querySelectorAll('.portfolio-grid .portfolio-item-link');
        ...
        filterButtons.forEach(button => {
            button.addEventListener('click', () => {
                ...
                portfolioItems.forEach(item => {
                    const actualItem = item.querySelector('.portfolio-item');
                    ...
                    if (shouldShow) {
                        ...
                        // Ẩn item với hiệu ứng
                        item.classList.remove('hide');
                        setTimeout(() => {
                            item.classList.add('hide');
                            // Sau khi animation kết thúc, ẩn hoàn toàn để không chiếm không gian
                            setTimeout(() => {
